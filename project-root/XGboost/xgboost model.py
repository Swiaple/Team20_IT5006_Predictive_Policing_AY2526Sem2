...
def get_neighbors(grid_idx: int, meta: dict) -> list:
    """
    返回 grid_idx 的 8 邻域索引。
    优先使用 meta 中已有字段；如果没有 active_grids/grid_remap，
    则默认网格是规则连续编号。
    """

    # 优先读取 n_rows / n_cols；如果没有就尝试 grid_h / grid_w
    if "n_rows" in meta and "n_cols" in meta:
        ...
        raise KeyError("meta.json 里找不到网格形状信息，例如 n_rows/n_cols 或 grid_shape")

    n_grids = meta["n_grids"]

    # 情况1：meta 里有 active_grids + grid_remap
...
def get_feature_config():
    ...
    config = {
        ...
        "rolling_windows": [6, 42, 180],   # mean
        "rolling_std_windows": [42, 180],  # std
...
def build_features_for_time(
    ...
    """
    返回当前 anchor 时间 t 对所有网格的特征矩阵
    shape = (G, F)

    这里的 t 表示“当前时刻”，目标是预测 t+horizon 的犯罪数
    """
...
def get_anchor_ranges(meta: dict, horizon: int):
    """
    anchor t 用来预测 t+horizon
    所以各 split 的 t 不能超过 split_end - horizon
    """
...
def get_crime_type_names(meta: dict):
    # 如果 meta 里有 type_names 就用它；否则给默认名字
...
def main(args):
    ...
    if meta["lookback"] < max_required:
        raise ValueError(
            f"meta['lookback']={meta['lookback']} 不足以支持当前最大窗口 {max_required}"
        )

    neighbor_list = build_neighbor_list(meta)

    # 1) 构造 train/val/test 数组
