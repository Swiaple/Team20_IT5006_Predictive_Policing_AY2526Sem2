...
def get_neighbors(grid_idx: int, meta: dict) -> list:
    ...
    Priority:
    1. Use active_grids + grid_remap if available in meta.
    2. Otherwise, assume grid_idx itself is a regular continuous grid index.
    """
    ...
    if "n_rows" in meta and "n_cols" in meta:
        ...
        raise KeyError("meta.json does not contain grid shape info such as n_rows/n_cols or grid_shape")
    ...
    # Case 2: assume grid_idx is itself a continuous regular grid index
...
def get_feature_config():
    ...
    42  = same time slot in the previous week
    ...
    config = {
        ...
        "neighbor_windows": [1, 6, 42],    # neighbor mean
        "horizon": 6,                      # predict the same time slot of the next day
    }
    return config
...
def build_feature_names(n_types: int, time_feature_cols: list, config: dict):
    ...
    # Build each feature group for all crime types
...
def compute_neighbor_mean_window(arr_tgc: np.ndarray, start: int, end: int, neighbor_list: list):
    """
    First compute the mean over the time window to get (G, C),
    then compute the neighborhood mean.
    """
...
def build_features_for_time(
    ...
    """
    Return the feature matrix for all grids at anchor time t.
    Shape = (G, F)

    Here, t is the current anchor time, and the target is
    the crime count at t + horizon.
    """
...
def get_anchor_ranges(meta: dict, horizon: int):
    ...
    Therefore, each split must satisfy:
    t <= split_end - horizon
    """
...
def build_split_arrays(
    ...
    # Use memmap to avoid loading huge arrays fully into RAM
...
def get_crime_type_names(meta: dict):
    ...
    # Default names for 5 common crime types
    default_names = [
        "THEFT",
        "BATTERY",
        "CRIMINAL_DAMAGE",
...
def train_one_crime_type(
    ...
    # Convert regression outputs to integer counts for accuracy calculation
    pred_val_int = np.rint(pred_val).astype(int)
    pred_test_int = np.rint(pred_test).astype(int)
    true_val_int = np.rint(true_val).astype(int)
    true_test_int = np.rint(true_test).astype(int)

    metrics = {
        "crime_type": crime_name,
        "val_mae": float(mean_absolute_error(true_val, pred_val)),
        "val_rmse": float(rmse(true_val, pred_val)),
        "val_acc": float(accuracy_score(true_val_int, pred_val_int)),
        "test_mae": float(mean_absolute_error(true_test, pred_test)),
        "test_rmse": float(rmse(true_test, pred_test)),
        "test_acc": float(accuracy_score(true_test_int, pred_test_int)),
    ...
    pred_df = pd.DataFrame({
        "y_true": true_test,
        "y_pred": pred_test,
        "y_true_int": true_test_int,
        "y_pred_int": pred_test_int,
        "match": (true_test_int == pred_test_int).astype(int)
    })
...
def main(args):
    ...
    time_features = np.load(processed_dir / "time_features.npy")    # (T, T_feat)
    ...
    # Check whether lookback is large enough for the maximum required window
    max_required = max(
        max(config["lags"]),
    ...
    if meta["lookback"] < max_required:
        raise ValueError(
            f"meta['lookback']={meta['lookback']} is not enough for the maximum required window {max_required}"
        )
    ...
    # 3) Train one model per crime type
    all_metrics = []
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Root data directory, e.g. ./data or ./data_v2"
    )
    parser.add_argument(
        "--output_dir",
        type=str,
